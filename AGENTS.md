...
- Invocation defaults to both model and user so a flow can cascade; mark `disable-model-invocation: true` only for a skill the model must never auto-fire and that no other skill needs to reach.
...
- Use [prose-standard](skills/agent/prose-standard/SKILL.md) for required prose coverage and editorial judgment.
...
For a semantic review of a non-trivial change (skill bodies, docs, notes), run [workspace-standards-review](skills/agent/workspace-standards-review/SKILL.md) before pushing.

## Quality Gate

Before finishing a skill change, run the platform, branding, and hard-path pass over the changed skill — [workspace-standards-review](skills/agent/workspace-standards-review/SKILL.md) carries the check and its command. Explain a legitimate match; remove or neutralize everything else before validation.
