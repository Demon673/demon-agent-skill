#!/usr/bin/env python3
"""Validate a SKILL.md against the Agent Skills frontmatter standard.

Accepts the standard fields: name, description, license, allowed-tools,
...
def validate_skill(skill_path):
    ...
    if unexpected:
        ...
        return False, f"Unexpected key(s) in SKILL.md frontmatter: {', '.join(sorted(unexpected))}. Allowed: {allowed}"
    ...
    if description:
        ...
        if len(description) > 1024:
            return False, f"Description is too long ({len(description)}). Maximum is 1024"
