...
**Every new Agent Note triggers a supersession check.** Search the active tree for older notes covering the same decision or mechanism, classify any full or partial supersession with [`archive-agent-notes`](../skills/archive-agent-notes/SKILL.md), and archive every qualifying implemented pair in the same PR. Keep partial supersessions active and cross-linked.
