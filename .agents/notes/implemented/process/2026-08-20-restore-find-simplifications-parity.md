...
- **Split the detailed sections into `references/`.** Rejected: the fusion note published the skill as self-contained, and each section is a workflow step every branch reaches, not conditional reference. The body therefore stays at 146 lines — above the conciseness default, but inside published precedent (`agent-doorbell` 165 lines, `merging-stacked-prs` 127 lines) and required by the parity specification.
