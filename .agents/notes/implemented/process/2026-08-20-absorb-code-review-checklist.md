# Agent Note: Absorb dsh-code-review checklist into workspace-standards-review
...
Add [`references/code-review-checklist.md`](../../../../skills/agent/workspace-standards-review/references/code-review-checklist.md) to `workspace-standards-review`: a generalized 1:1 port of `dsh-code-review` that keeps all six blocking requirements and manual checks while parameterizing deepseek-harness sources — standing orders and package rules, the defensive-patterns doc, the prose standard, the testing doc, the change-scope tool, the invariant and disposal conventions, and the translation rules and terminology. Add one pointer line in `workspace-standards-review/SKILL.md` so code-facing reviews load the reference.
...
- **Keep the code-facing checklist in `workspace-standards-review/SKILL.md`.** Rejected: the checklist is conditional reference that only code-facing reviews reach, and the skill stays concise when the material lives behind a pointer.
...
- `workspace-standards-review` now covers code-facing review semantics through `references/code-review-checklist.md`, reached by a pointer rather than always loaded.
