...
Write one when a bug is **subtle** (the mechanism is non-obvious and a careful engineer would re-derive it the hard way), **systemic** (the reason it escaped is a gap in checks, tooling, or conventions, not a one-off typo), and **costly to rediscover** (it cost real debugging time, and would cost it again). Link the guardrails (checks, `AGENTS.md` rules, Agent Notes) the post-mortem motivated.
...
| # | Title |
|---|---|
| _none yet_ | — add rows as incidents are written |
