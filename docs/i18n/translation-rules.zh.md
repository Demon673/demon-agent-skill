...
如何在本仓库的文档配对两侧之间进行翻译。配对契约——同等效力、来源侧、常规与扩展工作流的划分——在 [README.md](README.md)；这些规则约束对应版本的生成或更新方式。它们对人和 agent 同样有效。规则级别遵循 RFC 2119 用法：**MUST** / **MUST NOT** 是门禁或评审阻断项；**SHOULD** 需要说明理由才能偏离；**MAY** 可自行决定。
